...
function M.draw(ctx, modulator, width, height, state)
    ...
    if state.drag_segment > 0 and left_down then
        -- Vertical drag controls curve: drag DOWN = negative (ease-out/inward), drag UP = positive (ease-in/outward)
        local delta_y = norm_my - state.drag_start_y
        local new_curve = state.drag_start_curve - delta_y * 2  -- Inverted: drag down = inward
