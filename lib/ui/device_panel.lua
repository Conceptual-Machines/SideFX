...
--- Render modulator controls window (floating window for modulator parameters).
-- @param ctx ImGui context wrapper
-- @param guid string Device GUID
-- @param state_guid string State GUID for tracking expansion
-- @param modulators table Array of modulator FX objects
-- @param container TrackFX Container object
-- @param opts table Options (track, refresh_fx_list, etc.)
-- @return boolean Whether there was an interaction
local function render_modulator_controls_window(ctx, guid, state_guid, modulators, container, opts)
    local interacted = false

    -- Check if a modulator slot is expanded
    local expanded_slot_idx = expanded_mod_slot[state_guid]
    if expanded_slot_idx == nil then
        return false
    end

    -- Require imgui and state modules
        ...
        if expanded_modulator then
            ...
            if ok and param_count and param_count > 0 then
                ...
                if is_sync_mode then
                    ...
                    local sync_rate_names = {
                        "1/4", "1/4T", "1/8", "1/8T", "1/16", "1/16T",
                        "1/32", "1/32T", "1/64", "1/2", "1/2T", "1",
                        "2", "4", "1/4.", "1/8.", "1/16.", "1/32."
                    }
                    local current_sync_idx = math.floor(sync_rate * (#sync_rate_names - 1) + 0.5)
                    local current_sync_name = sync_rate_names[current_sync_idx + 1] or "1/4"
                    if ctx:begin_combo("##sync_rate", current_sync_name) then
                    ...
                    ctx:set_next_item_width(control_width)
                    local rate_hz_min = 0.01
                    local rate_hz_max = 20.0
                ...
                local current_trigger_name = trigger_names[current_trigger_idx + 1] or "Free"
                if ctx:begin_combo("##trigger_mode", current_trigger_name) then
                ...
                local advanced_expanded = state.modulator_advanced[expanded_modulator:get_guid()] or false
                if ctx:collapsing_header("Advanced##mod_advanced", nil) then
                    if not advanced_expanded then
                        state.modulator_advanced[expanded_modulator:get_guid()] = true
                    end

                    local current_trigger = math.floor(trigger_mode * 3 + 0.5)
                    ...
                    if advanced_expanded then
                        state.modulator_advanced[expanded_modulator:get_guid()] = false
                    end
                end
                ...
                -- Get current modulation links (using REAPER param.X.plink API at FX level)
                local active_links = {}
                for param_idx = 0, param_count - 1 do
                    -- Check if this parameter has an active plink
                    local link_active = expanded_modulator:get_named_config_param("param." .. param_idx .. ".plink.active")
                    if link_active and tonumber(link_active) == 1 then
                        local link_fx_str = expanded_modulator:get_named_config_param("param." .. param_idx .. ".plink.effect")
                        local link_param_str = expanded_modulator:get_named_config_param("param." .. param_idx .. ".plink.param")
                        if link_fx_str and link_param_str then
                            local link_fx_idx = tonumber(link_fx_str)
                            local link_param_idx = tonumber(link_param_str)
                            if link_fx_idx and link_param_idx then
                                table.insert(active_links, {
                                    mod_param_idx = param_idx,
                                    target_fx_idx = link_fx_idx,
                                    target_param_idx = link_param_idx
                                })
                            end
                        end
                    end
                ...
                -- Display active links with remove buttons
                if #active_links > 0 then
                    ctx:text("Active Links:")
                    for i, link in ipairs(active_links) do
                        -- Get target FX and param names
                        local track = opts.track or state.track
                        if track then
                            local ok_fx, target_fx = pcall(function() return track:get_track_fx(link.target_fx_idx) end)
                            if ok_fx and target_fx then
                                local ok_name, target_fx_name = pcall(function() return target_fx:get_name() end)
                                local ok_pname, target_param_name = pcall(function()
                                    return target_fx:get_param_name(link.target_param_idx)
                                end)
                                ...
                                if ok_name and ok_pname then
                                    ...
                                    if ctx:button("X##remove_link_" .. i, 20, 20) then
                                        -- Remove link
                                        expanded_modulator:set_named_config_param("param." .. link.mod_param_idx .. ".plink.active", "0")
                                        interacted = true
                                    end
                                end
                            end
                ...
                -- Get or create link state for this modulator
                local mod_guid = expanded_modulator:get_guid()
                if not state.mod_selected_target then
                    state.mod_selected_target = {}
                end
                local link_state = state.mod_selected_target[mod_guid]
                if not link_state then
                    ...
                    state.mod_selected_target[mod_guid] = link_state
                end

                -- Device selector dropdown
                ctx:set_next_item_width(control_width)
                local track = opts.track or state.track
                local device_name = link_state.device_fx and link_state.device_fx:get_name() or "Select Device..."
                if track and ctx:begin_combo("##link_device", device_name) then
                    -- Get all devices in current container (siblings)
                    local parent = expanded_modulator:get_parent_container()
                    if parent then
                        local ok, iter = pcall(function() return parent:iter_container_children() end)
                        if ok then
                            for child in iter do
                                local is_mod = fx_utils.is_modulator_fx(child)
                                if not is_mod then
                                    local child_name = child:get_name()
                                    local child_idx = child.pointer
                                    local is_selected = (link_state.device_fx_idx == child_idx)
                                    if ctx:selectable(child_name, is_selected) then
                                        link_state.device_fx = child
                                        link_state.device_fx_idx = child_idx
                                        link_state.param_idx = nil  -- Reset param selection
                                        interacted = true
                                    end
                                end
                            end
                ...
                -- Parameter selector dropdown (only if device is selected)
                if link_state.device_fx then
                    local ok, target_param_count = pcall(function() return link_state.device_fx:get_num_params() end)
                    if ok and target_param_count and target_param_count > 0 then
                        ctx:set_next_item_width(control_width)
                        local param_name = "Select Parameter..."
                        if link_state.param_idx ~= nil then
                            local ok_pname, pname = pcall(function()
                                return link_state.device_fx:get_param_name(link_state.param_idx)
                            end)
                            if ok_pname then
                                param_name = pname
                            end
                        end
                        if ctx:begin_combo("##link_param", param_name) then
                            for param_idx = 0, target_param_count - 1 do
                                local ok_pname, target_param_name = pcall(function()
                                    return link_state.device_fx:get_param_name(param_idx)
                                end)
                                if ok_pname then
                                    local is_selected = (link_state.param_idx == param_idx)
                                    if ctx:selectable(target_param_name, is_selected) then
                ...
                -- Add Link button (only if both device and param are selected)
                if link_state.device_fx and link_state.param_idx ~= nil then
                    if ctx:button("Add Link", control_width, 24) then
                        -- Create modulation link using param.X.plink API at FX level
                        -- Link the modulator's Output parameter (slider4, index 3) to the target
                        expanded_modulator:set_named_config_param("param.3.plink.active", "1")
                        expanded_modulator:set_named_config_param("param.3.plink.effect", tostring(link_state.device_fx_idx))
                        expanded_modulator:set_named_config_param("param.3.plink.param", tostring(link_state.param_idx))
                        expanded_modulator:set_named_config_param("param.3.plink.scale", "1.0")  -- Full scale

                        -- Reset link state
                        link_state.device_fx = nil
                        link_state.device_fx_idx = nil
...
function M.draw(ctx, fx, opts)
    ...
    if is_panel_collapsed then
        ...
        -- Calculate panel width: columns + combined sidebar (with modulators) + padding
        content_width = cfg.column_width * num_columns
        -- Combined sidebar is wider to accommodate modulator grid + all utility controls
        -- Width accounts for: Mix/Delta/Pan controls, horizontal gain fader, phase buttons, and 2×4 modulator grid
        local combined_sidebar_w = 320
        local sidebar_w = is_sidebar_collapsed and collapsed_sidebar_w or combined_sidebar_w
    ...
    if ctx:begin_child("panel_" .. guid, panel_width, panel_height, 0) then
            ...
            if is_sidebar_collapsed then
                ...
                if utility then
                    ...
                    if ok_g then
                        ...
                        -- Horizontal gain fader
                        local avail_w, _ = ctx:get_content_region_avail()
                        local fader_w = avail_w - 60  -- Leave space for label
                        local fader_h = 20

                        -- dB label on the left
                        local db_label = (math.abs(gain_db) < 0.1) and "0 dB" or (gain_db > 0 and string.format("+%.0f dB", gain_db) or string.format("%.0f dB", gain_db))
                        ctx:text(db_label)
                        ctx:same_line()

                        -- Horizontal slider
                        ctx:set_next_item_width(fader_w)
                        local gain_changed, new_gain_db = ctx:slider_double("##gain_fader_h", gain_db, -24, 24, "")
                        ...
                        if ctx:is_item_hovered() then
                            ctx:set_tooltip("Double-click to reset to 0dB")
                        end
                    end
                ...
                -- Modulator Grid Section
                ctx:spacing()
                ctx:spacing()
                ctx:separator()
                ctx:spacing()

                -- Center "Modulators" label
                local mod_text = "Modulators"
                local mod_text_w = r.ImGui_CalcTextSize(ctx.ctx, mod_text)
                center_item(mod_text_w)
                ctx:push_style_color(r.ImGui_Col_Text(), 0xAAAAAAFF)
                ctx:text(mod_text)
                ctx:pop_style_color()
        ...
        -- Render modulator controls window (floating window)
        local modulators = get_device_modulators(container)
        if render_modulator_controls_window(ctx, guid, state_guid, modulators, container, opts) then
            interacted = true
        end
