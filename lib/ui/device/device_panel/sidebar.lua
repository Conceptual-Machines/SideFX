...
function M.draw(ctx, fx, container, state_guid, sidebar_actual_w, is_sidebar_collapsed, cfg, opts, colors)
    ...
    if is_sidebar_collapsed then
        ...
        if has_mix or has_delta then
            ...
            ctx:with_table("mix_delta_" .. state_guid, 2, table_flags, function()
                ...
                if has_mix then
                    if draw_mix_column(ctx, container, mix_val, mix_idx) then
                        interacted = true
                    end
                end
                ...
                if has_delta then
                    if draw_delta_column(ctx, fx, delta_val, delta_idx) then
                        interacted = true
                    end
                end
            end)
