--[[
Collapsed Device Header Module - Draws button layout for collapsed device
With mix/delta (3x2): Row 1: ON | × | ◀  |  Row 2: UI | Mix | Delta
Without mix/delta (2x2): Row 1: ON | ◀  |  Row 2: UI | ×
]]

local M = {}

--- Draw collapsed device header with dynamic button layout
function M.draw(ctx, fx, container, state_guid, enabled, device_collapsed, opts, colors)
    ...
    -- Determine if we're in 3-column or 2-column mode
    local use_3_cols = show_mix_delta and (has_mix or has_delta)

    if use_3_cols then
        -- === 3x2 LAYOUT: ON | × | ◀  then  UI | Mix | Delta ===
